...
def merge(lst1, lst2):
    ...
    """
    "*** YOUR CODE HERE ***"
    if(len(lst1)==0 or len(lst2)==0):#极端情况
        return lst1+lst2
    elif(lst1[0]<lst2[0]):
        return [lst1[0]] +merge(lst1[1:],lst2)
    else:
        return [lst2[0]]+merge(lst1,lst2[1:])#有这一思路，就是比较两个中的第一个，然后继续比较，但一时想不出怎么写，
#就是只需除掉最小那个，然后返回两个列表，继续比较，中间过程有时我觉得不要在乎，因为你不一定知道他是怎么算的，只需知道思路，然后将其转为语句即可
...
def summation(n, term):
    ...
    def fib(n):
        if(n == 1):
            ...
            return term(n) + fib(n-1)
    return fib(n)
#好像没必要再弄多一个函数，只需要直接将其再调用一次summation即可
...
def count_palindromes(L):
    ...
    2
    """
    return sum(word.lower() == word.lower()[::-1] for word in L)
